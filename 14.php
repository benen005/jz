...
<?php }else{ ?>
	<p class="p1"><?php echo date('Y-m-d') . "竞赛足彩预测<br />"; ?></p> <div class="recommend_btn"><input name="secret" id="btn_secret" size="2" /><input type="button" value="推荐" name="btn_recommend" id="btn_recommend" /><input type="hidden" id="recommend_type" value="4" /></div>
<?php } ?>
...
for($i=0;$i<count($table);$i++){
	...
	$table2[] = render_red($table[$i], $p[$i][0]) . " ". sfp_310($table[$i], $p[$i][0]);  //第一轮渲染
	//echo $table[$i]."</br>";
	$sfp[$i] = sfp_310($table[$i], $p[$i][0]);
}
$t = array();
...
for($i=0;$i<count($table2);$i++){
	if($t[$i]){
		$table3[] = "<p class='p2'>" . render_red($table2[$i], $t[$i]) . sfp_310($table[$i], $t[$i]) . "</p>";  //第二轮渲染
		$sfp[$i] = $sfp[$i] . sfp_310($table[$i], $t[$i]);
	}
	else
		$table3[] = "<p class='p2'>" . render_red($table2[$i], $t[$i]) . "</p>"; 
